      ...
      try {
        ...
        if (data) {
          ...
          setOptions([
            allCategoriesOption,
            ...data.map(
              (category: { idcategory: string; categoryname: string }) => ({
          ...
          if (setCatId) {
            setCatId(
              data.map(
                (category: { idcategory: string }) => category.idcategory
              )
            );
          }
        }
      ...
      <div className="relative inline-flex rounded-lg bg-blue-500 text-white hover:bg-blue-600 items-center px-4 gap-2 border w-48">
        <div className="h-full">
          <button
            ...
            onClick={onToggle}
            className={`button-${
              isOpenProp ? "danger" : "success"
            } items-center text-white hover:bg-blue-600 inline-flex h-full rounded-r-md`}
          >
        ...
        <button
          type="button"
          onClick={onToggle}
          className="py-2 semibold-14 cursor-pointer items-center text-center"
          style={{
            maxWidth: "115px",
            overflow: "hidden",
        ...
        >
          <div className="flex items-center">
            {selectedOptionProp || "Semua Kategori"}
          </div>
        </button>
        {isOpenProp && (
