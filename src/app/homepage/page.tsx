...
export default function Homepage() {
  ...
  useEffect(() => {
    const getRole = async () => {
        ...
        if(user){
            const {data: roles, error} = await supabase.from('account').select('role').eq('email', user?.email);
            if(error){
            console.log(error);
            }
  ...
  return ( 
    ...
    />
    : (roleUser === 'admin' ||roleUser === 'kasir' || roleUser === 'inventaris' ?
      <Navbar 
            listOfNav={
                (roleUser === 'admin' ? NAV_ADMIN : (roleUser === 'kasir' ? NAV_KASIR : NAV_INVENTARIS))
            }
        />
    :
    ...
    <div className='bg-gradient-to-l from-sky-100 via-transparent to-sky-100'>
            ...
            <div className="flex items-center w-full lg:w-1/2">
                <div className="max-w-2xl my-15">
                    ...
                    <p className="py-5 px-2 text-base font-medium leading-normal text-gray-500 lg:text-base xl:text-base ">
                     sistem manajemen Point of Sales (POS) yang dirancang khusus untuk mendukung 
                     kelancaran operasional swalayan Bormy. Mine memberikan solusi 
                     yang intuitif dan efisien untuk membantu Anda mengelola transaksi, inventaris, 
                     dan aspek bisnis lainnya secara lebih efektif.
                    </p>
        ...
        <div className={`container p-8 my-2 mx-auto xl:px-0`}>
            <div className="flex flex-col justify-center">
                <div className="text-xl font-medium text-center text-gray-700">
                    Hai, {roleUser === 'norole' ? (
                        ...
                        roleUser === 'admin' || roleUser === 'kasir' || roleUser === 'inventaris' ? (
                        <span>
                        <span className="text-blue-500 font-semibold">{roleUser} </span>
                        Selamat bekerja hari ini!
                        </span>
                        ) : (
                            <span>
                            <span className="text-blue-500 font-semibold">Pengguna Baru! </span>
            ...
            <div className="flex flex-wrap justify-center gap-5 mt-10 md:justify-around bg-blue-600">
