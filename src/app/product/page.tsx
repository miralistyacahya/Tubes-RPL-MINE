...
export default function app() {
  ...
  const handleDelete = async (productname: string) => {
    try {
      ...
      setDataItem((prevData) =>
        prevData.filter((product) => product.productname !== productname)
      );
  ...
  const handleProductChange = (editedProduct: product) => {
    const updatedDataItem = dataItem.map((product) =>
      product.idproduct === editedProduct.idproduct
        ? { ...product, price: editedProduct.price, stock: editedProduct.stock }
        : product
    );
          ...
          <div className="grid grid-cols-2">
            ...
            <div className="justify-end flex flex-row pr-10 my-5 gap-4">
              ...
              <TambahData
                ...
                formTitle={["Nama Kategori"]}
                label="Kategori"
                icon={IconAddTop}
                colToBeValidate="categoryname"
              />
              <TambahDataDropdown
                tableName="product"
                columns={["productname", "category", "price", "stock"]}
                formTitle={["Nama Produk", "Nama Kategori", "Harga", "Stock"]}
                label="Produk"
                icon={IconAddTop}
                colToBeValidate="productname"
                dropdownCol="category"
                dropdownVal={cat}
                dropdownValId={catId}
              />
            </div>
              ...
              <p className="text-sm text-gray-700 pl-8">
                {!columns || !displayData || displayData.length === 0
                  ...
                  : `Showing ${pageVisited + 1} to ${
                      pageVisitedTo > totalCount ? totalCount : pageVisitedTo
