  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const { data: totalCountResponse } = await supabase
          .from("account")
          .select("count");
  ...
  const handleRoleChange = (editedAccount: account) => {
    ...
    const updatedDataItem = dataItem.map((account) =>
      account.username === editedAccount.username
        ? { ...account, role: editedAccount.role }
        : account
    );
  ...
  const handleDelete = async (username: string) => {
    try {
      ...
      setDataItem((prevData) =>
        prevData.filter((account) => account.username !== username)
      );
        ...
        <EditData
          ...
          fields={[
            ...
            },
            {
              ...
              key: "role",
              readOnly: false,
              options: ["admin", "inventaris", "kasir"],
              valNum: false,
            },
          ]}
          onSave={handleSave}
